		...
		try {
			...
			while(line!=null) {	
				...
				// extract detailed model parameters
					...
					System.out.println("Option " +para[0]+" is not recognized. INPUT FILE: "+parameter_file);
	...
	public void output_files(String output_file_folder) {
		try {
			...
			for(int k=0;k<this.num_gene_K;k++) {
				...
				for(int i=0;i<this.num_subj_N;i++)bw.write(","+this.exp_Z[k][i]);
				bw.write("\n");
			}
			bw.close();
			...
			for(int t=0;t<this.num_trait_T;t++) {
				...
				for(int i=0;i<this.num_subj_N;i++)bw.write(","+this.trait_Y[t][i]);
				bw.write("\n");
			}
			bw.close();
			...
			bw.write("#Gene_ID");
			for(int k=0;k<this.num_gene_K;k++)bw.write(","+this.gene_names[k]); 
			bw.write("\n");  // finished writing the header line
			// this is a triangle matrix
			for(int k1=0; k1<this.num_gene_K; k1++) {
				bw.write(this.gene_names[k1]); 
				for(int k2=k1; k2<this.num_gene_K; k2++) {
					bw.write(","+this.corr_KxK[k1][k2-k1]); 
				}bw.write("\n");
			}	
			bw.close();
			...
			for(int t1=0; t1<this.num_trait_T; t1++) {
				...
				for(int t2=t1; t2<this.num_trait_T; t2++) {
					bw.write(","+this.corr_TxT[t1][t2-t1]); 
				}bw.write("\n");
			}	
			bw.close();
			...
			for(int k=0; k<this.num_gene_K; k++) {
				...
				for(int m_index=0;m_index<m;m_index++) {
					...
					bw.write(","+chr+"_"+loc+","+this.asso_Kxm[k][m_index]);
				}bw.write("\n");
			}
			bw.close();
			...
			for(int t=0; t<this.num_trait_T; t++) {
				...
				for(int m_index=0;m_index<m;m_index++) {
					...
					int chr=chr_index+1;
					int loc=this.geno_variant_locs[chr_index][loc_index];
					bw.write(","+chr+"_"+loc+","+this.asso_Txm[t][m_index]);
				}bw.write("\n");
			}
			bw.close();
			...
			for(int t=0; t<this.num_trait_T; t++) {
				...
				if(this.causality_graph.containsKey(this.trait_names[t])) {
					...
					for(int s=0;s<sources.size();s++)bw.write(","+sources.get(s));
				}
				bw.write("\n");
			}
			for(int k=0; k<this.num_gene_K; k++) {

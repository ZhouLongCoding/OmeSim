...
public class CausalTerms {
	...
	// data that are extracted from the basic info 
	/*
	 * the first dimension is terms and the second is individuals. This is consistent to the dimensions 
	 * of geno_G, exp_Z, and trait_Y in the MainFrame, although inconsistent to X[][] in SpecificModels 
	 */
	 ...
	 * ID		10		5			gene1,gene2		gene1,gene2		trt1,trt2	1.0,0.5,4.0,2.0	additive	vc_value_inf	vc_value_noise
	 */
	public CausalTerms(String info_line) {
	...
	public void calculate_this_ID(MainFrame main_frame) {
		if(isGene(this.ID, main_frame)) { // this term is a gene, only which has cis
			...
			if(this.num_cis!=-1)
				this.geno_G_cis_var=this.sample_geno_vars(gene_itself, this.num_cis, main_frame);
		...
		if(this.num_trans!=-1)
			this.geno_G_trans_var=this.sample_geno_vars(this.trans_var_genes, this.num_trans, main_frame);
	 ...
	 */
	public static double[][] sample_geno_vars(String[] gene_names, int num_sampling_vars, MainFrame main_frame){
	 ...
	 *
	 */
	public static double[][] sample_geno_vars(int num_sampling_vars, MainFrame main_frame){
		if(num_sampling_vars<0) // for terms without genetic component, the number of variants are set to -1 by MainFrame.set_terms_model().
			return null;
		double[][] sampled_vars=new double[num_sampling_vars][];
		for(int m=0;m<num_sampling_vars;m++) {  // no deep clone; therefore no cost of memory.

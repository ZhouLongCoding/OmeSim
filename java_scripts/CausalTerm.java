...
public class CausalTerm {
	...
	public void calculate_this_ID(MainFrame main_frame) {
		...
		// Aggregate-then-interact: interaction models (epistatic/heterogenous/compensatory) receive
		// exactly two pooled super-terms (see combine_into_two_supers); linear/compound models receive
		// the full set of weighted contributors as before.
		boolean is_interaction = this.model.equals(SpecificModels._epistatic)
				|| this.model.equals(SpecificModels._compensatory)
				|| this.model.equals(SpecificModels._heterogenous);
		double[][] combined_X = is_interaction ? this.combine_into_two_supers() : this.combine_arrays_weighted();
...
//			}
	...
	/*
	 * Aggregate-then-interact input builder for interaction models (epistatic/heterogenous/compensatory).
	 *
	 * Instead of feeding all raw contributors to AND/OR/XOR (which degenerates over many inputs:
	 * AND -> almost always "false", OR -> almost always "true", and late-ordered expression/trait
	 * contributors get masked so they create no co-expression), we first ADDITIVELY POOL each side
	 * into one super-term and let the operator act on just two well-conditioned inputs:
	 *
	 *   Side A ("genetic")    = weighted sum of cis (weights[0]) and trans (weights[1]) variant columns.
	 *   Side B ("biological") = weighted sum of expression (weights[2]) and trait (weights[3]) columns.
	 *
	 * Each side is standardized (mean 0, sd 1). Because Side B is the sum of ALL expression/trait
	 * contributors, the focal term genuinely depends on (and co-expresses with) every one of them.
	 *
	 * If only one side exists (e.g., a genetics-only gene), that side's columns are split into two
	 * halves so the operator still receives two inputs.
	 *
	 * Returns X[sample_size][2].
	 */
	public double[][] combine_into_two_supers(){
		ArrayList<double[]> sideA=new ArrayList<double[]>(); // genetic columns (already weighted)
		ArrayList<double[]> sideB=new ArrayList<double[]>(); // biological columns (already weighted)
		int sample_size=-1;
		// genetic side: cis (weights[0]) + trans (weights[1])
		if(this.geno_G_cis_var!=null) sample_size=add_weighted_columns(sideA, this.geno_G_cis_var, this.weights[0], sample_size);
		if(this.geno_G_trans_var!=null) sample_size=add_weighted_columns(sideA, this.geno_G_trans_var, this.weights[1], sample_size);
		// biological side: expressions (weights[2]) + traits (weights[3])
		if(this.exp_Z_val!=null) sample_size=add_weighted_columns(sideB, this.exp_Z_val, this.weights[2], sample_size);
		if(this.trait_Y_val!=null) sample_size=add_weighted_columns(sideB, this.trait_Y_val, this.weights[3], sample_size);

		double[] superA, superB;
		if(!sideA.isEmpty() && !sideB.isEmpty()) {
			superA=sum_columns(sideA, sample_size);
			superB=sum_columns(sideB, sample_size);
		}else { // only one side present: split its columns into two halves to still give two inputs
			ArrayList<double[]> only = sideA.isEmpty()? sideB : sideA;
			ArrayList<double[]> half1=new ArrayList<double[]>(), half2=new ArrayList<double[]>();
			for(int c=0;c<only.size();c++) ((c%2==0)?half1:half2).add(only.get(c));
			if(half2.isEmpty()) half2.add(only.get(0)); // a single column: reuse it so the operator has two inputs
			superA=sum_columns(half1, sample_size);
			superB=sum_columns(half2, sample_size);
		}
		SpecificModels.standardization_with_weight(superA, 1.0);
		SpecificModels.standardization_with_weight(superB, 1.0);
		double[][] X=new double[sample_size][2];
		for(int i=0;i<sample_size;i++) { X[i][0]=superA[i]; X[i][1]=superB[i]; }
		return X;
	}

	/* Append each (column * weight) of a category array to target; returns the (validated) sample size. */
	private static int add_weighted_columns(ArrayList<double[]> target, double[][] category_arr, double weight, int sample_size){
		double w = Double.isNaN(weight)? 1.0 : weight; // a present category should have a finite weight; guard just in case
		for(int k=0;k<category_arr.length;k++) {
			double[] col=category_arr[k];
			if(sample_size==-1) sample_size=col.length;
			double[] weighted=new double[col.length];
			for(int i=0;i<col.length;i++) weighted[i]=col[i]*w;
			target.add(weighted);
		}
		return sample_size;
	}

	/* Element-wise sum of a list of equal-length columns. */
	private static double[] sum_columns(ArrayList<double[]> cols, int sample_size){
		double[] s=new double[sample_size];
		for(int c=0;c<cols.size();c++) {
			double[] col=cols.get(c);
			for(int i=0;i<sample_size;i++) s[i]+=col[i];
		}
		return s;
	}
	...
	/*
	 * DISABLED / PASS-THROUGH (2026-06-09): the structural trimming below is no longer applied.
	 * Under the aggregate-then-interact design, interaction models pool all of their contributors
	 * at compute time (see combine_into_two_supers), so every term must KEEP its full contributor
	 * list and the complete causality graph. This method now copies the architecture file through
	 * unchanged. The original 2-term trimming logic is retained below (guarded by `if(false)`) for
	 * reference and easy re-enabling.
	 *
	 * --- Original behavior (no longer active) ---
	...
	public static void adjust_causal_terms(String in_causal_file, String out_causal_file, MainFrame main_frame) {
		try {
			...
			bw.write("## Passed through from "+ in_causal_file +" unchanged (interaction-model contributors are pooled at compute time; see combine_into_two_supers).\n");
			...
			while(line!=null) {
				...
				if(false) { // DISABLED: no structural trimming. Contributors are pooled at compute time (combine_into_two_supers); every line passes through unchanged via the else branch below. Original trimming retained for reference.

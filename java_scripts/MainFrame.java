	...
	public void load_parameters(String parameter_file) {
		try {
			...
			while(line!=null) {	
				...
				else if(para[0].equals("Weights_Relative_Range"))this.weights_relative_range=Double.parseDouble(para[1]);
		...
		try {
			...
			while(line!=null) {
				...
				if(this.gene_locs.containsKey(para[0])) {
					...
					//para[0]=Gene_ID; para[1]=chr_value; para[2]=start_loc; para[3]=end_loc.
					String chrValue = para[1];
					if (!this.chr2index.containsKey(chrValue)) {
						continue;
					}
